...
public class BillableController {
	...
	@GetMapping(path = "/getexpi", produces = MediaType.APPLICATION_JSON_VALUE)
	public BillableResponse getExperience() {
		BillableResponse response = new BillableResponse();
		HashSet<Integer> list = service.getExperinceList();
		if (list == null) {
			response.setStatusCode(400);
			response.setMessage("failure");
			response.setDescription("data not successfully retrived..");
	...
	@GetMapping(path = "/expCountMap", produces = MediaType.APPLICATION_JSON_VALUE)
	public BillableResponse map() {
		BillableResponse response = new BillableResponse();
		Map<Integer, Integer> map1 = service.getExpCount();
		if (map1 == null) {
			response.setStatusCode(400);
			response.setMessage("failure");
			response.setDescription("data not successfully retrived..");
