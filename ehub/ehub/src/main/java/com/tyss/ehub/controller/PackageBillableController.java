...
public class PackageBillableController {
	...
	@PostMapping(path ="/addPackage", consumes =MediaType.APPLICATION_JSON_VALUE 
	...
	@GetMapping(path ="/revenuList", produces = MediaType.APPLICATION_JSON_VALUE)
	public PackageResponse revenuList() {
		PackageResponse response = new PackageResponse();
		response = service.getRevenuList();
		if (response!= null) {
			response.setStatusCode(201);
			response.setMessage("success");
			response.setDescription("data successfully retrived..");
	...
	@GetMapping(path ="/profit", produces = MediaType.APPLICATION_JSON_VALUE)
	public PackageResponse overAllProfit() {
		
		PackageResponse response = new PackageResponse();
		Double profit = service.profit();
